    ...
    <div className="card p-4">
          ...
          <div key={d.key} className="flex-1 flex flex-col items-center gap-1">
            <div className="w-full flex gap-0.5 items-end" style={{ height: 68 }}>
              <div className="flex-1 rounded-t chart-bar"
                style={{ height: `${(d.income / maxVal) * 100}%`, minHeight: d.income > 0 ? 2 : 0, background: 'linear-gradient(to top, #34C759, #30D158)', opacity: 0.75, animationDelay: `${i*60}ms` }} />
              <div className="flex-1 rounded-t chart-bar"
                style={{ height: `${(d.expenses / maxVal) * 100}%`, minHeight: d.expenses > 0 ? 2 : 0, background: 'linear-gradient(to top, #FF453A, #FF6961)', opacity: 0.65, animationDelay: `${i*60+30}ms` }} />
            </div>
      ...
      <div className="flex gap-4 mt-3">
        <div className="flex items-center gap-1.5">
          <div className="w-2 h-2 rounded-sm" style={{ background: '#34C759', opacity: 0.75 }} />
          <span className="text-[10px]" style={{ color: 'rgba(255,255,255,0.4)' }}>Income</span>
        </div>
        <div className="flex items-center gap-1.5">
          <div className="w-2 h-2 rounded-sm" style={{ background: '#FF453A', opacity: 0.7 }} />
...
export default function FinanceTabs({ trips, expenses, vehicles }: { trips: Trip[]; expenses: Expense[]; vehicles: Vehicle[] }) {
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      {tab === 'overview' && (
        <div className="flex flex-col gap-2 animate-fade-up">
          ...
          <div className="grid grid-cols-3 gap-2">
            <div className="rounded-[28px] p-3" style={{ background: 'rgba(52,199,89,0.07)', border: '1px solid rgba(52,199,89,0.18)' }}>
            ...
            <div className="rounded-[28px] p-3" style={{ background: 'rgba(255,69,58,0.07)', border: '1px solid rgba(255,69,58,0.18)' }}>
            ...
            <div className="rounded-[28px] p-3" style={{ background: '#111111', border: '1px solid rgba(255,255,255,0.07)' }}>
      ...
      {tab === 'income' && (
        <div className="flex flex-col gap-2 animate-fade-up">
          {filteredTrips.map((trip, i) => (
            <div key={trip.id} className="pressable rounded-[28px] p-3.5 flex items-center justify-between"
      ...
      {tab === 'expenses' && (
        <div className="flex flex-col gap-2 animate-fade-up">
          <Link href="/finance/expenses/new" className="pressable flex items-center justify-center gap-2 rounded-[28px] py-3.5 text-sm"
          ...
          {filteredExpenses.length > 0 && (
            <div className="rounded-[28px] p-4" style={{ background: '#111111', border: '1px solid rgba(255,255,255,0.07)' }}>
          ...
          {filteredExpenses.map((e, i) => (
            <div key={e.id} className="pressable rounded-[28px] p-3.5 flex items-center justify-between"
      ...
      {tab === 'vehicles' && (
        <div className="flex flex-col gap-2 animate-fade-up">
          {vehicles.map((vehicle, i) => {
            ...
            return (
              <div key={vehicle.id} className="rounded-[28px] p-4" style={{ background: '#111111', border: '1px solid rgba(255,255,255,0.07)' }}>
