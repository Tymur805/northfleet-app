...
const NAV = [
  { href: '/',            label: 'Home',     icon: (a:boolean) => <svg width="20" height="20" fill="none" viewBox="0 0 24 24" stroke={a?'#0A84FF':'rgba(255,255,255,0.35)'} strokeWidth={1.8}><path strokeLinecap="round" strokeLinejoin="round" d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6"/></svg> },
  { href: '/vehicles',    label: 'Fleet',    icon: (a:boolean) => <svg width="20" height="20" fill="none" viewBox="0 0 24 24" stroke={a?'#0A84FF':'rgba(255,255,255,0.35)'} strokeWidth={1.8}><path strokeLinecap="round" strokeLinejoin="round" d="M8 17h8M4 17l1.5-6h13L20 17M7 11l1-4h8l1 4"/></svg> },
  { href: '/trips',       label: 'Trips',    icon: (a:boolean) => <svg width="20" height="20" fill="none" viewBox="0 0 24 24" stroke={a?'#0A84FF':'rgba(255,255,255,0.35)'} strokeWidth={1.8}><path strokeLinecap="round" strokeLinejoin="round" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"/></svg> },
  { href: '/reminders',   label: 'Alerts',   icon: (a:boolean) => <svg width="20" height="20" fill="none" viewBox="0 0 24 24" stroke={a?'#0A84FF':'rgba(255,255,255,0.35)'} strokeWidth={1.8}><path strokeLinecap="round" strokeLinejoin="round" d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9"/></svg> },
  { href: '/maintenance', label: 'Service',  icon: (a:boolean) => <svg width="20" height="20" fill="none" viewBox="0 0 24 24" stroke={a?'#0A84FF':'rgba(255,255,255,0.35)'} strokeWidth={1.8}><path strokeLinecap="round" strokeLinejoin="round" d="M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.065 2.572c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.572 1.065c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.065-2.572c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z"/><path strokeLinecap="round" strokeLinejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/></svg> },
  { href: '/finance',     label: 'Finance',  icon: (a:boolean) => <svg width="20" height="20" fill="none" viewBox="0 0 24 24" stroke={a?'#0A84FF':'rgba(255,255,255,0.35)'} strokeWidth={1.8}><path strokeLinecap="round" strokeLinejoin="round" d="M12 6v12m-3-2.818l.879.659c1.171.879 3.07.879 4.242 0 1.172-.879 1.172-2.303 0-3.182C13.536 12.219 12.768 12 12 12c-.725 0-1.45-.22-2.003-.659-1.106-.879-1.106-2.303 0-3.182s2.9-.879 4.006 0l.415.33M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg> },
]
...
export default function FloatingActions() {
  ...
  // Mic position (absolute pixels from top-left)
  ...
  return (
    <>
      {/* ── AI OVERLAY ── */}
      {overlay && (
        <div className="fixed left-1/2 -translate-x-1/2 w-full max-w-xl px-3 z-50 animate-slide-up"
          style={{ bottom: NAV_H + 80, pointerEvents: 'none' }}>
          <div className="glass-overlay rounded-[24px] p-4" style={{ pointerEvents: 'auto' }}>
            {overlay.userText && (
              <p className="text-[11px] mb-2 italic" style={{ color: 'rgba(255,255,255,0.35)' }}>"{overlay.userText}"</p>
            )}
            {overlay.loading
              ? <div className="flex items-center gap-2"><span className="animate-pulse-dot w-1.5 h-1.5 rounded-full bg-blue-400" /><p className="text-sm" style={{ color: 'rgba(255,255,255,0.5)' }}>Thinking…</p></div>
              ...
              <div className="flex items-center gap-3 mt-3">
                ...
                {overlay.copyText && (
                  <button onClick={() => navigator.clipboard.writeText(overlay.copyText!)}
                    ...
                    style={{ background: 'rgba(10,132,255,0.18)', color: '#0A84FF', border: '1px solid rgba(10,132,255,0.25)' }}>
                    Copy message
                  </button>
                )}
              </div>
      ...
      {/* ── PLUS MENU ── */}
      {plusOpen && (
        <div data-plus className="fixed z-50 flex flex-col gap-1.5 px-3 animate-slide-up" style={{ bottom: NAV_H + 72, left: 0 }}>
          {QUICK.map((a, i) => (
            <Link key={a.href} href={a.href} onClick={() => setPlusOpen(false)}
              className={`pressable flex items-center gap-3 px-4 py-3 rounded-[28px] delay-${i*50}`}
              style={{ background: 'rgba(22,22,22,0.95)', backdropFilter: 'blur(24px)', border: '1px solid rgba(255,255,255,0.1)', boxShadow: '0 8px 32px rgba(0,0,0,0.6)' }}>
      ...
      {/* ── PLUS BUTTON (bottom-left, fixed) ── */}
      <button data-plus
        onClick={() => { setPlusOpen(p => !p); setOverlay(null) }}
        className="spring"
        style={{
          position: 'fixed', bottom: NAV_H + 8, left: 20, zIndex: 55, width: 52, height: 52,
          borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center',
          background: 'rgba(28,28,30,0.92)', backdropFilter: 'blur(20px)',
          border: '1px solid rgba(255,255,255,0.13)',
          boxShadow: plusOpen ? '0 0 0 2px rgba(10,132,255,0.3), 0 8px 24px rgba(0,0,0,0.5)' : '0 8px 24px rgba(0,0,0,0.5)',
        }}>
      ...
      {/* ── MIC BUTTON (draggable, bottom-right) ── */}
      <button
        ...
        style={{
          ...
          border: `1px solid ${listening ? 'rgba(255,69,58,0.4)' : 'rgba(10,132,255,0.4)'}`,
          boxShadow: listening
            ? '0 0 0 4px rgba(255,69,58,0.2), 0 8px 24px rgba(255,69,58,0.35)'
            : '0 0 0 0px rgba(10,132,255,0), 0 8px 24px rgba(10,132,255,0.4)',
          cursor: 'grab',
          touchAction: 'none',
          userSelect: 'none',
        }}>
        ...
        {NAV.map(item => {
          ...
          return (
            <Link key={item.href} href={item.href}
              className="pressable flex flex-col items-center justify-center gap-0.5 w-10 h-10 rounded-[20px]"
              style={{ background: active ? 'rgba(10,132,255,0.1)' : 'transparent' }}>
              {item.icon(active)}
              {active && <span className="w-1 h-1 rounded-full" style={{ background: '#0A84FF' }} />}
            </Link>
          )
        })}
      </nav>
