...
const navItems = [
  ...
  { href: '/maintenance', icon: (a: boolean) => <svg width="22" height="22" fill="none" viewBox="0 0 24 24" stroke={a ? '#3b82f6' : '#71717a'} strokeWidth={1.8}><path strokeLinecap="round" strokeLinejoin="round" d="M11.42 15.17L17.25 21A2.652 2.652 0 0021 17.25l-5.877-5.877M11.42 15.17l2.496-3.03c.317-.384.74-.626 1.208-.766M11.42 15.17l-4.655 5.653a2.548 2.548 0 11-3.586-3.586l6.837-5.63m5.108-.233c.55-.164 1.163-.188 1.743-.14a4.5 4.5 0 004.486-6.336l-3.276 3.277a3.004 3.004 0 01-2.25-2.25l3.276-3.276a4.5 4.5 0 00-6.336 4.486c.091 1.076-.071 2.264-.904 2.95l-.102.085m-1.745 1.437L5.909 7.5H4.5L2.25 3.75l1.5-1.5L7.5 4.5v1.409l4.26 4.26m-1.745 1.437l1.745-1.437m6.615 8.206L15.75 15.75M4.867 19.125h.008v.008h-.008v-.008z" /></svg> },
          ...
          for (const action of data.actions) {
            ...
            if (action.type === 'create_reminder') {
              try {
                ...
                localStorage.setItem('nf_reminders', JSON.stringify([...existing, action.reminder]))
              } catch {}
            }
          }
  ...
  return (
    <>
      ...
      <div className="flex items-center justify-between px-4 pb-2 pt-1">
        {/* Plus — big circle popping up */}
        <button
          data-plus
          onClick={() => { setPlusOpen(p => !p); setOverlay(null) }}
          className="w-14 h-14 rounded-full bg-zinc-800 border border-zinc-700 flex items-center justify-center text-white shadow-lg active:scale-95 transition-transform -translate-y-5 shrink-0"
        >
        ...
        {/* Nav icons — icons only, no labels */}
        <div className="flex items-center justify-center gap-1 flex-1 px-1">
          {navItems.map((item) => {
            const active = pathname === item.href || (item.href !== '/' && pathname.startsWith(item.href))
            return (
              <Link key={item.href} href={item.href}
                className={`w-9 h-9 flex items-center justify-center rounded-xl transition-colors ${active ? 'bg-blue-500/10' : ''}`}>
                {item.icon(active)}
              </Link>
            )
          })}
        ...
        {/* Mic — big circle popping up */}
        <button
          onClick={handleMic}
          className={`w-14 h-14 rounded-full flex items-center justify-center shadow-xl active:scale-95 transition-all -translate-y-5 shrink-0 ${
