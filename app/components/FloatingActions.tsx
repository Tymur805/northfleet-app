...
const QUICK = [
  { label: 'Add Vehicle', href: '/vehicles/new',        icon: <svg width="16" height="16" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M8 17h8M4 17l1.5-6h13L20 17M7 11l1-4h8l1 4"/></svg> },
  { label: 'Log Trip',    href: '/trips/new',            icon: <svg width="16" height="16" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"/></svg> },
  { label: 'Log Service', href: '/maintenance/new',      icon: <svg width="16" height="16" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.065 2.572c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.572 1.065c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.065-2.572c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z"/><path strokeLinecap="round" strokeLinejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/></svg> },
  { label: 'Add Expense', href: '/finance/expenses/new', icon: <svg width="16" height="16" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M12 6v12m-3-2.818l.879.659c1.171.879 3.07.879 4.242 0 1.172-.879 1.172-2.303 0-3.182C13.536 12.219 12.768 12 12 12c-.725 0-1.45-.22-2.003-.659-1.106-.879-1.106-2.303 0-3.182s2.9-.879 4.006 0l.415.33M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg> },
]

// Glossy glass button style
const glassBtn = (active: boolean, listening: boolean): React.CSSProperties => ({
...
const glassPlusBtn = (open: boolean): React.CSSProperties => ({
  width: 64, height: 64, borderRadius: '50%',
  display: 'flex', alignItems: 'center', justifyContent: 'center',
  background: open
    ? 'linear-gradient(145deg, rgba(255,60,30,0.52) 0%, rgba(255,26,0,0.32) 55%, rgba(130,0,0,0.38) 100%)'
...
export default function FloatingActions() {
  ...
  const defaultMicBottom = NAV_H + 14
    ...
    <>
      ...
      {overlay && (
        <div className="fixed left-1/2 -translate-x-1/2 w-full max-w-xl px-4 z-50 animate-slide-up"
          style={{ bottom: NAV_H + 90, pointerEvents: 'none' }}>
      ...
      {/* ── PLUS MENU ── */}
      {plusOpen && (
        <div data-plus className="fixed z-50 flex flex-col gap-1.5 px-4 animate-slide-up" style={{ bottom: NAV_H + 82, left: 0 }}>
          {QUICK.map((a, i) => (
      ...
      {/* ── PLUS BUTTON (glossy glass, bottom-left) ── */}
      <button data-plus
        onClick={() => { setPlusOpen(p => !p); setOverlay(null) }}
        className="spring"
        style={{ ...glassPlusBtn(plusOpen), position: 'fixed', bottom: NAV_H + 14, left: 20, zIndex: 55 }}>
        <svg width="24" height="24" fill="none" viewBox="0 0 24 24"
          stroke={plusOpen ? 'white' : 'rgba(255,255,255,0.8)'} strokeWidth={2.5}
          style={{ transform: plusOpen ? 'rotate(45deg)' : 'rotate(0deg)', transition: 'transform 0.3s cubic-bezier(0.34,1.56,0.64,1)' }}>
          <path strokeLinecap="round" strokeLinejoin="round" d="M12 4v16m8-8H4"/>
        </svg>
      </button>

      {/* ── MIC BUTTON (glossy glass, draggable) ── */}
      <button
        onPointerDown={onMicPointerDown}
        className="spring animate-float"
        style={{ ...glassBtn(false, listening), ...micStyle }}>
        {listening
          ? <svg width="22" height="22" fill="white" viewBox="0 0 24 24"><rect x="6" y="6" width="12" height="12" rx="2"/></svg>
